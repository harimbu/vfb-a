  ...
  <div>
    ...
    <v-menu offset-y v-else-if="!$store.state.fireUser">
      <template v-slot:activator="{ on }">
        <v-btn dark v-on="on" icon><v-icon>mdi-account</v-icon></v-btn>
      </template>
      <v-card>
        <v-card-title>로그인</v-card-title>
        <v-card-actions>
          <v-btn @click="signInWithGoogle"><v-icon left>mdi-google</v-icon>구글로그인</v-btn>
        </v-card-actions>
      </v-card>
    </v-menu>
    ...
    <v-menu offset-y v-else>
      ...
      <v-card>
        ...
        <v-card-actions>
          <v-btn @click="signOut" block><v-icon left>mdi-logout</v-icon>로그아웃</v-btn>
        </v-card-actions>
      </v-card>
    </v-menu>
  </div>
